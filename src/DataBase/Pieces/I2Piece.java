...
public class I2Piece extends Piece {
    private static final int[][] pieceRep = new int[][]{{1},{1}};
    ...
    public I2Piece(int state) {
        super("I2", new int[][]{{1},{1}}, false, 2, 2,state);
    }
    ...
    @Override
    public Piece clone() {
        return new I2Piece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (this.corners == null) { //if the corners have not been computed since last rotation, search for them
            ...
            for(int[][] rep: getPermutations()) {
                ...
                this.corners.add(Corner.copyOf(current_corners));
