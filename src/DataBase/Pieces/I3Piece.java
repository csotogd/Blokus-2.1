...
public class I3Piece extends Piece {
    ...
    private static final int[][] pieceRep = new int[][]{{1},{1},{1}};
    ...
    @Override
    public Piece clone() {
        return new I3Piece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (I3Piece.corners == null) { //if the corners have not been computed since last rotation, search for them
            I3Piece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                I3Piece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return I3Piece.corners;
    }
}
