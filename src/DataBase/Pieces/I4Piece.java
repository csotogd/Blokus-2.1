...
public class I4Piece extends Piece {
    ...
    @Override
    public Piece clone() {
        return new I4Piece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (I4Piece.corners == null) { //if the corners have not been computed since last rotation, search for them
            I4Piece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                I4Piece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return I4Piece.corners;
    }
}
