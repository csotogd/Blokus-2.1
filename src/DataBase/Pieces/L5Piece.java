...
public class L5Piece extends Piece {
    ...
    private static final int[][] pieceRep = new int[][]{{1,0},{1,0},{1,0},{1,1}};
    ...
    public L5Piece(int state) {
        super("L5", new int[][]{{1,0},{1,0},{1,0},{1,1}}, true, 4, 8,state);
    }
    ...
    @Override
    public Piece clone() {
        return new L5Piece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (L5Piece.corners == null) { //if the corners have not been computed since last rotation, search for them
            L5Piece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                L5Piece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return L5Piece.corners;
    }
}
