...
public class NPiece extends Piece {
    ...
    private static final int[][] pieceRep = new int[][]{{1,0},{1,0},{1,1},{0,1}};
    ...
    @Override
    public Piece clone() {
        return new NPiece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (NPiece.corners == null) { //if the corners have not been computed since last rotation, search for them
            NPiece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                NPiece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return NPiece.corners;
    }
}
