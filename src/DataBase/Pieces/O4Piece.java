...
public class O4Piece extends Piece {
    ...
    private static final int[][] pieceRep =  new int[][]{{1,1},{1,1}};
    ...
    public O4Piece(int state) {
        super("O4", new int[][]{{1,1},{1,1}}, false, 1, 1,state);
    }
    ...
    @Override
    public Piece clone() {
        return new O4Piece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (O4Piece.corners == null) { //if the corners have not been computed since last rotation, search for them
            O4Piece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                O4Piece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return O4Piece.corners;
    }
}
