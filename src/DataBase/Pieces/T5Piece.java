...
public class T5Piece extends Piece {
    ...
    private static final int[][] pieceRep = new int[][]{{1,1,1},{0,1,0},{0,1,0}};
    ...
    public T5Piece(int current_state) {
        super("T5", new int[][]{{1,1,1},{0,1,0},{0,1,0}}, true, 4, 4,current_state);
    }
    ...
    @Override
    public Piece clone() {
        return new T5Piece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (T5Piece.corners == null) { //if the corners have not been computed since last rotation, search for them
            T5Piece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                T5Piece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return T5Piece.corners;
    }
}
