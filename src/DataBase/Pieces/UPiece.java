...
public class UPiece extends Piece {
    ...
    private static final int[][] pieceRep = new int[][]{{1,0,1},{1,1,1}};
    ...
    @Override
    public Piece clone() {
        return new UPiece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (UPiece.corners == null) { //if the corners have not been computed since last rotation, search for them
            UPiece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                UPiece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return UPiece.corners;
    }
}
