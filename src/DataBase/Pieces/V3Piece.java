...
public class V3Piece extends Piece {
    ...
    private static final int[][] pieceRep = new int[][]{{1,1}, {1,0}};
    ...
    @Override
    public Piece clone() {
        return new V3Piece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (V3Piece.corners == null) { //if the corners have not been computed since last rotation, search for them
            V3Piece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                V3Piece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return V3Piece.corners;
    }
}
