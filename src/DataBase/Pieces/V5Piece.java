...
public class V5Piece extends Piece {
    ...
    private static final int[][] pieceRep = new int[][]{{1,0,0},{1,0,0},{1,1,1}};
    ...
    public V5Piece(int state) {
        super("V5", new int[][]{{1,0,0},{1,0,0},{1,1,1}}, true,4, 4,state);
    }
    ...
    @Override
    public Piece clone() {
        return new V5Piece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (V5Piece.corners == null) { //if the corners have not been computed since last rotation, search for them
            V5Piece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                V5Piece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return V5Piece.corners;
    }
}
