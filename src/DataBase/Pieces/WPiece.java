...
public class WPiece extends Piece {
    ...
    private static final int[][] pieceRep =new int[][]{{1,0,0},{1,1,0},{0,1,1}};
    ...
    @Override
    public Piece clone() {
        return new WPiece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (WPiece.corners == null) { //if the corners have not been computed since last rotation, search for them
            WPiece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                WPiece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return WPiece.corners;
    }
}
