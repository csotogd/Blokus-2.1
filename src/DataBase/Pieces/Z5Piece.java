...
public class Z5Piece extends Piece {
    ...
    private static final int[][] pieceRep = new int[][]{{1,1,0},{0,1,0},{0,1,1}};
    ...
    public Z5Piece(int state) {
        super("Z5", new int[][]{{1,1,0},{0,1,0},{0,1,1}}, true, 2, 4,state);
    }
    ...
    @Override
    public Piece clone() {
        return new Z5Piece(current_state);
    }

    @Override
    public List<List<Corner>> getCorners() {
        if (Z5Piece.corners == null) { //if the corners have not been computed since last rotation, search for them
            Z5Piece.corners = new ArrayList<>();
            for(int[][] rep: getPermutations()) {
                ...
                Z5Piece.corners.add(Corner.copyOf(current_corners));

            }
        }

        return Z5Piece.corners;
    }
}
