...
//-------------------------------------------General methods-------------------------------------------------------
    ...
    private void getPercentageOfWinners(){
        int one=0;
        int two=0;
        int three=0;
        int four=0;
        int[] nbrOfWinners= new int[4];
        for (GeneticPlayer winner : winners){
           nbrOfWinners[winner.getNumber()-1]= nbrOfWinners[winner.getNumber()-1]+1;
/*
            if(winner.getNumber()==1)
                one++;
            if(winner.getNumber()==2)
                two++;
            if(winner.getNumber()==3)
                three++;
            if(winner.getNumber()==4)
                four++;
...
/*
        nbrOfWinners[0]=one;
        nbrOfWinners[1]=two;
        nbrOfWinners[2]=three;
        nbrOfWinners[3]=four;
*/
        System.out.println("nbr of winners: "+winners.size());
        for (int i=1; i<=4; i++){
            float percent=(nbrOfWinners[i-1]);
            percent=percent/winners.size();
            System.out.println("player "+i+" : "+ percent);
        }
...
//------------------------------------------Reproduction methods---------------------------------------------------
    ...
    private GeneticPlayer reproduce(GeneticPlayer father, GeneticPlayer mother){
        ...
        //return this.reproduceweightsByChromosomesColapsePhases(father,mother);
        return this.reproduceweightsByChromosomesDifferentPhases(father, mother);
        //return this.reproduceweightsBySmallPerturbationDiffPhases(father, mother);
    }
    ...
    private GeneticPlayer reproduceweightsBySmallPerturbationDiffPhases(GeneticPlayer father, GeneticPlayer mother){
        Random r = new Random();
        float[][] weightsFather = father.getWeightsAsArray();
        float[][] weightsMother = mother.getWeightsAsArray();
        ...
        for (int i = 0; i < GeneticPlayer.NUMBER_OF_STRATEGIES; i++){
            ...
            //phase 0
            if (r.nextBoolean())
                kidsWeights[0][i] = weightsFather[0][i]+(r.nextFloat()*2-1)*0.025f;
            else
                kidsWeights[0][i] = weightsMother[0][i]+(r.nextFloat()*2-1)*0.025f;

            //phase 1
            if (r.nextBoolean())
                kidsWeights[1][i] = weightsFather[1][i]+(r.nextFloat()*2-1)*0.025f;
            else
                kidsWeights[1][i] = weightsMother[1][i]+(r.nextFloat()*2-1)*0.025f;

            //phase 2
            if (r.nextBoolean())
                kidsWeights[2][i] = weightsFather[2][i]+(r.nextFloat()*2-1)*0.025f;
            else
                kidsWeights[2][i] = weightsMother[2][i]+(r.nextFloat()*2-1)*0.025f;
        }
     ...
     * @return A new individual that will be introduced in the population
     */
    private GeneticPlayer reproduceweightsByChromosomesDifferentPhases(GeneticPlayer father, GeneticPlayer mother){
        Random r = new Random();
        float[][] weightsFather = father.getWeightsAsArray();
        float[][] weightsMother = mother.getWeightsAsArray();
        ...
        for (int i = 0; i < GeneticPlayer.NUMBER_OF_STRATEGIES; i++){
            ...
            //phase 1
            if (r.nextBoolean())
                kidsWeights[1][i] = weightsFather[1][i];
            else
                kidsWeights[1][i] = weightsMother[1][i];

            //phase 2
            if (r.nextBoolean())
                kidsWeights[2][i] = weightsFather[2][i];
            else
                kidsWeights[2][i] = weightsMother[2][i];
        }
     ...
     * @return A new individual that will be introduced in the population
     */
    private GeneticPlayer reproduceweightsByChromosomesColapsePhases(GeneticPlayer father, GeneticPlayer mother){
        Random r = new Random();
        float[][] weightsFather = father.getWeightsAsArray();
        float[][] weightsMother = mother.getWeightsAsArray();
        ...
        for (int i=0; i<3; i++) {//set every weight of kid
            ...
            kidsWeights[phase] = kidsWeightsforDesiredPhase;
                ...
                //for the other two phases weights are kept constant
                    ...
                    kidsWeights[i]= this.computedWeightsPhase0;
            }
        }
    ...
    private void mutateWeightsOnlyDesiredPhase(GeneticPlayer player, int alteredPhase){
        ...
        //Every weight has an equal opportunity to be mutated
            ...
            for (int mutateWeight = 0; mutateWeight < weights[0].length; mutateWeight++) {
                if (r.nextDouble() <= mutationChance) {
                    //We can always change this.
                    weights[alteredPhase][mutateWeight] = r.nextFloat();
                }
